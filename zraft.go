...
type raftNode struct {
	proposeC    <-chan string            // 接收 messages (k,v) 信道
	confChangeC <-chan raftpb.ConfChange // 接收集群 config changes 信道
...
func newRaftNode(id int,
	...
	rc := &raftNode{
		...
		peers:       peers,
		join:        join,
		waldir:      fmt.Sprintf("zraftexample-%d", id),      // wal文件夹名 zraftexample-{id}
		snapdir:     fmt.Sprintf("zraftexample-%d-snap", id), // snapdir文件夹名 zraftexample-{id}-snap
...
func (rc *raftNode) loadSnapshot() *raftpb.Snapshot {
	...
	// 从文件中加载最新的raftpb.Snapshot
	snapshot, err := rc.snapshotter.Load()
	if err != nil && err != snap.ErrNoSnapshot {
		log.Fatalf("zraftexample: error loading snapshot (%v)", err)
	}
	return snapshot
}
...
func (rc *raftNode) openWAL(snapshot *raftpb.Snapshot) *wal.WAL {
	if !wal.Exist(rc.waldir) {
		if err := os.Mkdir(rc.waldir, 0750); err != nil {
			log.Fatalf("zraftkv: cannot create dir for wal (%v)", err)
		}

		w, err := wal.Create(rc.waldir, nil)
		if err != nil {
			log.Fatalf("zraftkv: create wal error (%v)", err)
		}
		w.Close()
	}
	...
	if err != nil {
		log.Fatalf("zraftkv: error loading wal (%v)", err)
	}

	return w
}

// replayWAL 把 WAL条目存储到raft实例
...
func (rc *raftNode) publishSnapshot(snapshotToSave raftpb.Snapshot) {
	...
	rc.commitC <- nil // 给kvstore发送消息,通知kvstore加载snapshot
...
func (rc *raftNode) serveChannels() {
	...
	for {
		...
		case <-ticker.C:
			...
			// 存储raft条目到wal, 然后通过commit channel发布
